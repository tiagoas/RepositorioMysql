...
--INICIO NOTA FISCAL DE SAIDA
		...
		SELECT A.NUM_PEDIDO,A.ID_CLIENTE,A.COD_PAGTO
		FROM  PED_VENDAS A
		WHERE A.NUM_PEDIDO=@DOCTO
	...
	--FINAL CURSOR DETALHES
	   ...
	   --ATUALIZANDO TOTAL NFE
		...
		UPDATE PED_VENDAS  SET SITUACAO='F' WHERE NUM_PEDIDO=@DOCTO;
  ...
  ELSE IF @TIP_MOV='E' 
		...
		SELECT A.NUM_PEDIDO,A.ID_FOR,A.COD_PAGTO
		FROM  PED_COMPRAS A
		WHERE A.NUM_PEDIDO=@DOCTO
		AND A.SITUACAO<>'F' --FINALIZADO
	...
	--FINAL CURSOR DETALHES
	 ...
	 --ATUALIZANDO TOTAL NFE
		UPDATE NOTA_FISCAL SET TOTAL_NF=@TOTAL_NFE WHERE NUM_NF=@NUM_NF;
		--ATUALIZADO STATUS PARA FECHADO NFE
		UPDATE PED_COMPRAS  SET SITUACAO='F' WHERE NUM_PEDIDO=@DOCTO;
...
SELECT TOP 1 * FROM PED_VENDAS
SELECT TOP 1 * FROM PED_COMPRAS
